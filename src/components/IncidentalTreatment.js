...
const IncidentalTreatment = () => {
  ...
  const ProgressBar = () => (
    <div className="w-full bg-gray-200 rounded-full h-2 mb-6 shadow-inner">
      <div 
        className="bg-gradient-to-r from-orange-400 via-orange-500 to-orange-600 h-2 rounded-full shadow-lg transition-all duration-500 ease-in-out"
  ...
  const PageIndicator = () => (
    <div className="flex justify-center space-x-2 mb-6">
      { [1,2,3].map((p) => (
        <div key={p} className={`w-3 h-3 rounded-full transition-all duration-300 ${
          currentPage === p ? 'bg-orange-500 scale-125 shadow-md' : 'bg-gray-300'
        }`} />
      )) }
    </div>
  );
  ...
  const input =
    "w-full p-4 text-base border-2 border-gray-200 rounded-xl focus:border-orange-400 focus:ring-2 ring-orange-200 outline-none shadow-sm transition-all placeholder-gray-400";
          ...
          <div className="fixed bottom-5 left-1/2 -translate-x-1/2 z-30 w-11/12 max-w-md bg-white/90 backdrop-blur-xl rounded-3xl shadow-2xl p-1">
            <div className="flex gap-3">
              <button 
                type="button" 
                onClick={handleReset}
                className="flex-1 py-4 px-6 bg-gradient-to-r from-slate-400 to-slate-500 text-white font-bold rounded-2xl shadow-lg hover:shadow-xl active:scale-95 transition-all text-lg border-0"
              >
                🔄 Reset
              </button>
              <button 
                type="submit" 
                className="flex-1 py-4 px-6 bg-gradient-to-r from-indigo-500 to-blue-600 text-white font-bold rounded-2xl shadow-lg hover:shadow-xl active:scale-95 transition-all text-lg border-0"
              >
                🚀 Submit
              </button>
            </div>
